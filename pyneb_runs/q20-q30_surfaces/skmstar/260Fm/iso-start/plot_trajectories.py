...
if __name__ == "__main__":
    ...
    E_const_arr = [0,0.5,1]
    path_files_asym = sorted(glob.glob(f'shift-*/{nuc}_path_Asymmetric_Mass_True_Econst_*.txt'))
    path_files_sym = sorted(glob.glob(f'shift-*/{nuc}_path_Symmetric_1_Mass_True_Econst_*.txt'))
    path_files_sym2 = sorted(glob.glob(f'shift-*/{nuc}_path_Symmetric_2_Mass_True_Econst_*.txt'))
    ...
    iso_ind = utilities.SurfaceUtils.find_local_minimum(EE,searchPerc=[0.60,0.60],returnOnlySmallest=False)
    ...
    gs_ind = utilities.SurfaceUtils.find_local_minimum(EE_fine,searchPerc=[0.15,0.15],returnOnlySmallest=True)
    gs_coord_fine = np.array((xx_fine[gs_ind],yy_fine[gs_ind])).T
    
    iso_ind_fine = utilities.SurfaceUtils.find_local_minimum(EE_fine,searchPerc=[0.60,0.60],returnOnlySmallest=False)
    ...
    for i,fpath in enumerate(path_files_sym):
        ...
        print(f'Action = {action}')
        print(f'Exit point = {data[-1]}')
    for i,fpath in enumerate(path_files_sym2):
        print(fpath)
        data = np.loadtxt(fpath,delimiter=',',skiprows=1) 
        ax.plot(data[:,0],data[:,1],'-.',ms=24,color=color_arr[i], zorder=10, clip_on=False)
        ax.plot(data[:,0][-1],data[:,1][-1],'s',ms=4,color=color_arr[i],zorder=10, clip_on=False)
