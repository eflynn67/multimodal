...
class FragmentYields:
    ...
    def __call__(self,mode='python-new',sigmaA=3,sigmaZ=2,sigmaZFinal=0.5):
        ...
        if mode in ['python-new','python-jhilam']:
            ...
            maxList = dfByA.max(axis=1)
            minList = dfByA.min(axis=1)
            dfByA['max'] = maxList
            dfByA['min'] = minList
            # print(dfByA)
            
            dfByZ = dfByZ.set_index('Z')
            maxList = dfByZ.max(axis=1)
            minList = dfByZ.min(axis=1)
            dfByZ['max'] = maxList
            dfByZ['min'] = minList
...
def plot_localization(df,col):
    ...
    fig, ax = plt.subplots()
    cf = ax.contourf(rr,zz,arrToPlot,cmap="Spectral_r")
...
for f in locFiles:
    ...
    with open(f'frag_result/charge_{mode}_Eshift_{Eshift}_{center_method}_centers.txt', 'w') as f:
        f.write('Neutron\n')
        f.write(f'{locs[0]} \n')
        f.write('Proton\n')
        f.write(f'{locs[1]}')
