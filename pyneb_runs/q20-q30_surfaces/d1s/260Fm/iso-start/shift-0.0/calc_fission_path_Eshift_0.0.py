...
if __name__ == "__main__":
    ...
    iso_ind = utilities.SurfaceUtils.find_local_minimum(EE,searchPerc=[0.80,0.20],returnOnlySmallest=False)
    ...
    print(E_iso_grid_arr[iso_sorted])
    E_iso_grid  = E_iso_grid_arr[iso_sorted[1]]
    iso_coord_grid = iso_coord_grid_arr[iso_sorted[1]]
    #print(iso_sorted)
    #print(E_iso_grid)
...
############################################
    ...
    iso_ind_fine = utilities.SurfaceUtils.find_local_minimum(EE_fine,searchPerc=[0.35,0.2],returnOnlySmallest=False)
    ...
    #print(iso_coord_fine_arr[iso_sorted])
    ...
    plt.plot(iso_coord_fine[0],iso_coord_fine[1],'x',color='red',ms=10)
    plt.xlim([0,300])
    plt.ylim([0,32])
    plt.xlabel(r'$Q_{20}$ (b)')
    plt.ylabel(r'$Q_{30}$ (b$^{3/2}$)')
    plt.title(f'USING interpolator {nuc}')
    plt.show()
...
###############################################################################
    ...
    ### define initial path
    R0 = iso_coord_grid
    # the asymm path is just to demonstrate that it hugs the otl.
    RNArr = [[225,16]]#,[190,0]]
    #RNArr = [[200,2]]
    ...
    if NIterations_const > 0:
        ...
        for p,RN in enumerate(RNArr):
            ...
            FireParams = {"dtMax":.1,"dtMin":10**(-6),"nAccel":10,"fInc":1.1,"fAlpha":0.99,\
                 "fDecel":0.5,"aStart":0.1,"maxmove":np.array([.8,.8])}
    ...
    for p,RN in enumerate(RNArr):
        ...
        FireParams = {"dtMax":.1,"dtMin":10**(-6),"nAccel":10,"fInc":1.1,"fAlpha":0.99,\
             "fDecel":0.5,"aStart":0.1,"maxmove":np.array([.9,.9])} 
