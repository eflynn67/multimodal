...
###############################################################################
    ...
    dt = .1
    NIterations_const =8000
    NIterations_var =10000
    
    ### define initial path
    R0 = gs_coord_grid
    # run with no constant inertia initialization yielded a larger action.
    RNArr = [[220,10],[235,0]]
    #RNArr = [[132,0],[132,0]]
    ...
    for p,RN in enumerate(RNArr):
        ...
        FireParams = {"dtMax":.1,"dtMin":10**(-6),"nAccel":10,"fInc":1.1,"fAlpha":0.99,\
             "fDecel":0.5,"aStart":0.1,"maxmove":np.array([.5,.01])} 
