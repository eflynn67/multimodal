...
if __name__ == "__main__":
    ...
    path_files = sorted(glob.glob(f'gs-start/{nuc}_path_*_Mass_True_*.txt'))
    ...
    fig, ax = plt.subplots(1,1)
    im = ax.contourf(uniq_coords[0],uniq_coords[2],EE[:,6].T.clip(0,50),levels= 100,
                      extend='both',cmap='Spectral_r')
    #ax.clabel(cs, cs.levels, inline=True, fontsize=8)
    otl1 = ax.contour(uniq_coords[0],uniq_coords[2],EE[:,6].T.clip(0,50),levels=[0],colors='white',linewidths=1.5)
