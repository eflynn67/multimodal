...
mass_dataFiles = sorted(glob.glob(f'iso-start/nlfs_yields/frag_result/mass_*.dat'))
charge_dataFiles = sorted(glob.glob(f'iso-start/nlfs_yields/frag_result/charge_*.dat'))
print(mass_dataFiles)
...
names = ['Asymm1','Asymm2','Compact','Elongated']
massFig, massAx = plt.subplots()
chargeFig, chargeAx = plt.subplots()
color_arr = ['lime','blue','red','orange']
for i in np.arange(0,len(mass_dataFiles)):
    ...
    massAx.set(xlabel=r'$A$',title='Mass Yield (%) SkM* 3d')
    
    #c1 = charge[:charge.shape[0]//2]
    #c2 = np.flip(charge[charge.shape[0]//2:],axis=0)
    
    chargeAx.plot(charge[:,0],charge[:,1],label=f'{names[i]}',color=color_arr[i])
    ...
    chargeAx.fill_between(charge[:,0],charge[:,1],charge[:,2],hatch='/',facecolor='none',zorder=200,linewidth=0.0,\
                        edgecolor=color_arr[i])
    chargeAx.set(xlabel=r'$A$',title='Charge Yield (%) SkM* 3d')
    
    #data = pd.read_csv(dataFile,nrows=169,sep='\s+',skiprows =1)
    #plt.plot(data.to_numpy()[:,0],data.to_numpy()[:,1],label=f'Eshift = {names[i]}')
massAx.legend()
chargeAx.legend()
#massFig.savefig(f'{nuc}_{mode}_mass_fragments_SkMstar_{center_method}.pdf')
#chargeFig.savefig(f'{nuc}_{mode}_charge_fragments_SkMstar_{center_method}.pdf')
